...
class WheelVelocityConverter:
    ...
    def cmd_vel_callback(self, twist_msg):
        ...
        self.send_speed_request('right', right_wheel_velocity)
        self.send_speed_request('left', left_wheel_velocity)

        # print('requests not sent')

        rospy.sleep(0.1)

    def send_speed_request(self, wheel, velocity):
        url = f"{self.server_url}/set_speed?wheel={wheel}&speed={velocity:.2f}"
        print(url)
        try:
            response = requests.get(url)
            response.raise_for_status()  # Raise an exception for HTTP errors
            rospy.loginfo(f"Successfully sent speed {velocity:.2f} for {wheel} wheel.")
        except requests.exceptions.RequestException as e:
            rospy.logerr(f"Failed to send speed for {wheel} wheel. Error: {e}")
