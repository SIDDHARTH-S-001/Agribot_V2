...
while True:
    ...
    retR, cornersR = cv2.findChessboardCorners(grayR,(7,3),None)  # Define the number of chess corners (here 7 by 3) we are looking for with the right Camera
    retL, cornersL = cv2.findChessboardCorners(grayL,(7,3),None)  # Same with the left camera
    ...
    if (retR == True) & (retL == True):
        ...
        cv2.drawChessboardCorners(grayR,(7,3),corners2R,retR)
        cv2.drawChessboardCorners(grayL,(7,3),corners2L,retL)
