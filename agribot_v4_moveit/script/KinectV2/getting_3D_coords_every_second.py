...
class KinectDepthProcessor:
    def __init__(self):
        ...
        self.detectNames = ["sports ball"]

        # For frame rate (FPS)
        self.prev_time = 0
        self.fps = 0

        # Timer for capturing images every second
        self.capture_timer = rospy.Timer(rospy.Duration(1), self.capture_images)

    def load_camera_parameters(self, camera_matrix_file, distortion_matrix_file):
        # Loads camera matrix and distortion coefficients from text files along with updating the necessary variables
    ...
    def pixel_to_point(self, u, v, depth):
        ...
        # x -> horizontal axis, y -> vertical axis and z -> distance from camera (coordinates convention)
        ...
        for r in results:
            ...
            for box in boxes:
                ...
                if cls in self.detectNames: # we are interested only in a few classes
                    ...
                    if confidence >= 0.3:  # Check if confidence is above 0.5
                        ...
                        if depth != 0:
                            ...
                            cv2.putText(img, coord_text, tuple((org[0], org[1] + 30)), font, fontScale, color, thickness)
    ...
    def run(self):
        rospy.spin()  # Spin until shutdown
...
def main():
    ...
    camera_matrix_file = '/home/siddharth/capstone_ws/src/Agribot_V2/agribot_v4_moveit/script/KinectV2/camera_matrix_kinect.txt'
    distortion_matrix_file = '/home/siddharth/capstone_ws/src/Agribot_V2/agribot_v4_moveit/script/KinectV2/distortion_coefficients_kinect.txt'
